...
class StringFactory(DataFactory):
  ...
  def __call__(self, tensor_spec: tf.TensorSpec,
    ...
    def rand_str():
      return ''.join(
          rgn.choice(
              ...
              size=(rgn.randint(self._min_chars, self._max_chars))))
    ...
    return np.array([rand_str() for _ in range(np.prod(shape, dtype=np.int32))
                    ]).reshape(shape).astype(tf.string.as_numpy_dtype)
...
class FeatureFactory:
  ...
  def generate(self, rgn: np.random.RandomState) -> Any:
    print(f'DEBUG: generate: df={self.data_factory}')
  ...
  def __init__(self,
    ...
    for feature_name, feature in self._features.items():
      ...
      if isinstance(data_factory, dict):
        print(f'DEBUG: WEIRD, for {feature_name} we have a dict {data_factory}')
      self._feature_factories[feature_name] = FeatureFactory(
          feature.get_tensor_info(), data_factory)
    super().__init__()

  def generate_example(self,
                       rgn: np.random.RandomState) -> MutableMapping[str, Any]:
    print(f'DEBUG: feature feactories: {self._feature_factories}')
    return {
